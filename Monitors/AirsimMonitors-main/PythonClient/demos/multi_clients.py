...
class SurveyNavigator:
    def __init__(self, args):
        ...
        for i in range(1,self.droneNum):
            self.droneClients["client{}".format(i)] = airsim.MultirotorClient()
            cur = self.droneClients["client"+str(i)]
            cur.enableApiControl(True, "Drone"+str(i))
            cur.armDisarm(True, "Drone"+str(i))
            print(cur.listVehicles())
        
   

    def start(self):
       #print(self.droneClients)
        # print("arming the drone...")
        # for i in self.droneClients:
        #     i.armDisarm(True,vehicle_name="Drone"+str(i))
        ...
        time.sleep(2)
        for i in range(1,self.droneNum):
            self.droneClients["client"+str(i)].takeoffAsync().join()
        # for i in self.droneClients: 
        #     #sync takeoff
        #     self.droneClients[i].join()
        ...
        for i in range(1,self.droneNum):
            self.drones["Drone{}".format(i)] =self.client.moveToPositionAsync(0, 0, z, self.velocity,vehicle_name="Drone"+str(i))
        ...
        for i in range(1,self.droneNum):
            self.drones["Drone{}".format(i)] =self.client.moveToPositionAsync(x, -self.boxsize, z, self.velocity,vehicle_name="Drone"+str(i))
        for i in self.drones: 
            self.drones[i].join()
